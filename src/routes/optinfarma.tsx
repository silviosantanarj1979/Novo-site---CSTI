...
function OptinfarmaPage() {
  return <main id="inicio" className="inner-page optinfarma-page">
    ...
    <section className="optinfarma-flow section"><div className="shell"><div className="heading"><p className="eyebrow ink">COMO O PROJETO AVANÇA</p><h2>Da demonstração à loja da sua drogaria.</h2></div><div className="optinfarma-steps">{[["01","Diagnóstico","Entendemos marca, lojas, catálogo, sistemas e operação.","COMPREENDER A REALIDADE"],["02","Escopo","Definimos recursos, integrações, regras e responsabilidades.","TRANSFORMAR IDEIAS EM PLANO"],["03","Experiência","Aplicamos identidade, conteúdo e jornadas do negócio.","DESENHAR PARA O CLIENTE"],["04","Integrações","Conectamos o que for tecnicamente validado no projeto.","UNIR TODA A OPERAÇÃO"],["05","Validação","Testamos compra, atendimento, segurança e dispositivos.","ENTRAR NO AR COM CONFIANÇA"],["06","Evolução","Acompanhamos dados e oportunidades após a entrada em operação.","MELHORAR CONTINUAMENTE"]].map(([n,t,d,outcome])=><article key={n}><span>{n}</span><b>{outcome}</b><div><h3>{t}</h3><p>{d}</p></div></article>)}</div></div></section>
